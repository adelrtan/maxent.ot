...
where $f_k(SR_i, UR_i)$ is the number of violations of constraint $k$ incurred by mapping underlying $UR_i$ to surface $SR_i$, and $Z_w(UR_i)$ is a normalization term defined as 

$$Z(UR_i) = \Sigma_{y\in \mathcal{Y}(UR_i)} exp(- \Sigma_{k=1}^m w_k f_k(SR, UR_i))$$

where $y \in \mathcal{Y}(UR_i)$ is the set of surface realizations of input $UR_i$.

We'll walk through the MaxEnt tableaux in *Figure 2* to illustrate how these formulas are applied.

```{r, echo=FALSE, fig.cap="Fig 2: Two MaxEnt tableaux.", out.width = '85%'}
knitr::include_graphics("../man/figures/maxEnt_tableau_3.JPG")
```
...
To calculate the penalty for each UR-SR pair, we take the weighted sum of its constraint violations.
...
The penalty for other UR-SR pairs are similarly calculated.
...
$Z$ is the sum of the MaxEnt scores of all SRs arising from a particular UR.
...
$$P(\text{sr2-2}|\text{ur-2}) = \frac{0.449}{0.490}$$
...
curve(exp(-x), from=0, to=5,
      col="white", xlab="penalty", ylab="MaxEnt score")    # Hack to set up window

# Add horizontal & vertical lines at y=0 & x=0
abline(h=0, col="gray")
abline(v=0, col="gray")

curve(exp(-x), from=0, to=5.25, col="blue", add=TRUE)      # Extend curve to to R-edge of window
```
...
Hence, the relationship between penalty and probability is clear (though the graph shows the MaxEnt score in place of probability).
...
In other words, the value of each of the elements of $w$ (*i.e.* the weight of each individual constraint) is the only variable available for the model to adjust. 
For now, we'll ignore the second term on the right-hand side of the equation (the bias term).
...
Here's the log likelihood equation again (bias term removed since we're not using biases here):

$$LL_w(D) = \Sigma_{i=1}^n ln P(y_i|x_i; w)$$

Applying the equation to *M1* ($w_{M1}$ refers to *M1*'s weights): 
$$LL_{w_{M1}}(D) = ln(.5) + ln(.5) + ln(1) = -1.386$$
Applying the equation to *M2* ($w_{M2}$ refers to *M2*'s weights): 
$$LL_{{w_{M2}}}(D) = ln(.25) + ln(.75) + ln(1) = -2.367$$
...
In the previous section (&sect;2), we ignored the second term, so log likelihood depended only upon the likelihood of the training data under the current weights.
...
The User, however, is advised that when multiple values for the same parameter is specified, the algorithm decides which one to use according to the chart in *Figure 4*.
...
#### Retrieving the weight for 1 constraint
...
We'll take a closer look at both the function `predict_probabilities()`, and the structure of the test file in &sect;4 below.
...
Notice that when $T=1$, the generalized model is equivalent to the simplified one defined in &sect;1.3.
