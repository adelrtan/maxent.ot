...
# 1 Overview
## 1.1 Why a Maximum Entropy (MaxEnt) model?
...
Thus, multiple models (Stochastic OT (Boersma 1997), Partial Ordering OT (Antilla 1997), Harmonic Grammar (Legendre, Miyata & Smolensky 1990), among others) have been put forward to augment the basic structure of OT, to allow the modeling of probabilistic data.
MaxEnt (Goldwater & Johnson 2003) is a one such model. 
It works to maximize the likelihood of data by adjusting constraint weights.
...
Since Con1 outranks Con2, [sr1-1] is the sole winner.
Hence [sr1-1] is the sole winner, and is predicted to be the only SR for this particular UR (*i.e.* a categorical outcome).
By the very same logic, the SR [sr2-1] is the sole realization of the UR /ur-2/ (*i.e.* Con1 prefers [sr2-1]).
Notice that Con2 plays no role in deciding the outcome. 

## 1.2 The MaxEnt model defined
...
The predicted probability of an SR candidate given its UR depends on the weights of the constraints, calculated according to the following formula:

$P(SR_i|UR_i; w) = \frac{1}{Z_w(UR_i)} exp(- \Sigma_{k=1}^mw_k f_k(SR_i, UR_i))$

where $f_k(SR_i, UR_i)$ is the number of violations of constraint $k$ incurred by mapping underlying $UR_i$ to surface $SR_i$, $Z_w(UR_i)$ is a normalization term defined as 

$Z(UR_i) = \Sigma_{y\in \mathcal{Y}(UR_i)} exp(- \Sigma_{k=1}^m w_k f_k(SR, UR_i))$

where $y \in \mathcal{Y}(UR_i)$ is the set of observed surface realizations of input $UR_i$.

To better understand how these formulas are applied, we'll walk through the MaxEnt tableaux in *Figure 2*.

```{r, echo=FALSE, fig.cap="Fig 2: Two MaxEnt tableaux.", out.width = '85%'}
knitr::include_graphics("../man/figures/maxEnt_tableau_2.JPG")
```


## 1.3 Calculating MaxEnt probabilities: Step-by-step
Notice that this MaxEnt tableau has the same URs, SRs, constraints, and constraint violation profiles as the OT tableaux above.
However, instead of being ranked, these constraints now have weights (*e.g.* Con1 weight = 1.6; Con2 weight = 0.8).
There are also four additional columns tagged on to the right of the tableaux, which will be used to illustrate how the predicted probabilities are computed.

An important variable that shows up in the formulas above is: $f_k(SR, UR_i)$. 
This refers to the constraint violation profile for a single UR-SR pair (yellow box).

To get the penalty for each UR-SR pair, we take the weighted sum of its constraint violations.
For example, the UR-SR pair $<$/ur-2/, [sr2-2]$>$ picks up 1 violation of Con1 and another violation of Con2.
The weight sum (green box) is thus: 

$weight_{Con1} \times 1 + weight_{Con2} \times 2 = 1.6 \times 1 + 0.8 \times 2 = 3.2$ 

The penalty for other SRs are similarly calculated.
For example, the penalty for [sr2-1] is 

$weight_{Con1} \times 0 + weight_{Con2} \times 1 = 1.6 \times 0 + 0.8 \times 1 = 0.8$

The penalty has also been known as the "harmony". 

The next column gives the value of $e^{-penalty}$.
The Reader may have also encountered this term being called the "MaxEnt score" or the "potential" in other descriptions of MaxEnt.
...
$Z$ allows the MaxEnt scores of all SRs arising from a single UR to be normalized.
For example, $Z($ur-2$)$ is the sum of the MaxEnt scores of all SRs of /ur-2/ (blue box).

The predicted probability is shown in the final column.
The probability of a particular SR for a given UR is its MaxEnt score divided by the normalization constant, $Z$.
For example, the probability of [sr2-2] given /ur-2/ (magenta box) is 

$P($sr2-2$|$ur-2$) = \frac{0.449}{0.490}$
...
## 1.4 Characteristics of the MaxEnt model
...
**As a particular SR's penalty increases, its probability decreases.**
The graph below shows how the MaxEnt score varies with penalty.
TODO: INSERT GRAPH HERE!!!
Recall that the probability of *SR~i~* given its UR, *UR~i~*, is directly proportional to its MaxEnt score:

$P(SR_i|UR_i) \propto M_{UR_i, SR_i}$

where $M_{UR_i, SR_i}$ is the MaxEnt score of the UR-SR pair $<$*UR~i~*, *SR~i~*$>$.
We know this because 

$P(SR_i|UR_i) = \frac{1}{Z(UR_i)} M_{UR_i, SR_i}$.

Hence, the relationship between penalty and probability is clear even though the graph shows the MaxEnt score in place of probability.

The graph also shows that SR candidates with infinitely high penalties still receive a MaxEnt score greater than zero.
...
We will introduce the final formulation of the probability distribution when we discuss the `predict_probabilities()` function, where it turns out that temperature can be a useful tool in **adjusting model fit to particular psycholinguistic tasks**.
Since we ignore temperature for now, the current formulation of the MaxEnt model is in fact an instance of a log-linear model. 
...
It has been observed in phonological phenomena, and has also been reported experimentally (Breiss 2020). 
...
data_file <- system.file("extdata", "sample_data_file.txt", package = "maxent.ot")

# Take a look at structure of sample data
...
* Column 3: Frequency
  ...
  + Token counts are recommended if user intends to use the function `compare_models` down the line to compare multiple trained models. In particular, relative frequency won't work with the `aic_c` & `bic` methods of `compare_models`.
...
``` {r}
# Take a look at model we've fit (no biases used)
simple_model
```
...
## 2.2.1 Pulling up specific model attributes 
We can pull up any specific model attribute that we'd like.   
...
Likewise, we can pull up any other model attribute. For example, here's the log likelihood:
...
The issue of overfitting and the introduction of priors to reduce it will be taken up in $\S3.3$.
But first, let's gain a better understanding of how log likelihood guides training.


## 2.3 Fitting the model to data: Explained
By default, the `optimize_weights()` function maximizes the log likelihood of the training data by changing the constraint weights.
The log likelhood of the training data is:

$LL_w(D) = \Sigma_{i=1}^n ln P(y_i|x_i; w) - \Sigma_{k=1}^m \frac{(w_k - \mu_k)^2}{2\sigma_k^2}$

where $LL_w(D)$ is the log likelihood of the training data, $D$ is the training data, and $w$ is the vector of constraint weights.
In other words, the values of each of the elements of $w$ (*i.e* each individual constraint) are the only variables available for the model to adjust. 
For now, we'll ignore the second term (the bias term).

#### What guides the weight changes?
Recall that the learner takes in a distribution over training examples $D$ (the first three columns of the table below).
...
The learner's goal is to maximize the log likelihood of this data by producing a distribution over these three training examples that matches the observed distribution as closely as possible. 
The learner adjusts the weights with this goal in mind.
Let's imagine we have two trained models *M1* & *M2*, whose predicted distributions P(output|input) over the training examples are shown in the final two columns of the table above.
For ease of comparison, the observed relative frequencies are shown in the column P(Output|Input).
Log likelihoods may range from negative infinity to 0, with higher log likelihoods being better.

$logLike_{M1} = ln(.5) + ln(.5) + ln(1) = -1.386$

$logLike_{M2} = ln(.25) + ln(.75) + ln(1) = -2.367$

The log likelihood of *M1* is higher because its predicted distribution over observed data points is closer to that of the observed distribution.
...
Hence, the learner's objective is to maximize the log likelihood of the data; that is, finding weights that produce a predicted distribution that is as close to the observed one as possible.


# 3 Fitting the model with priors: `optimize_weights()`

Here's the general formula for log likelihood, repeated here for ease of reference:

$LL_w(D) = \Sigma_{i=1}^n ln P(y_i|x_i; w) - \Sigma_{k=1}^m \frac{(w_k - \mu_k)^2}{2\sigma_k^2}$

In the previous section ($\S2$), we ignored the second term, so log likelihood depended only upon the likelihood of the training data under the current weights.
In this section, we'll make use of the second term.
This term is also known by several names such as the "bias term", the "prior", and in special cases, the "regularization term".
The bias term allows us to nudge the learner towards particular weights by introducing an added cost for weights that differ from the ones that the user specifies.
...
Worked examples using the bias term for regularization and for modeling learning biases are provided in $\S3.3$ and $\S3.4$ respectively.

## 3.1 Unpacking the math behind the bias term
The bias term has three variables: $w_k$, $\mu_k$ and $\sigma_k$.
...
Every constraint, $k$, whose weight $w_k$ differs from $\mu_k$ will thus incur an additional cost.
This cost is subtracted from the log likelihood, and thus makes it smaller (notice the negative sign preceding the second term).
Recall that for log likelihood, the higher the better.
The cost incurred depends on two factors: (1) Distance from the ideal weight; (2) Tolerance for weight discrepancy.

(1) Cost increases as the distance from the ideal weight increases. 
The distance from the ideal weight is $w_k - \mu_k$. 
For example, let's set the ideal weight for constraint $k$ at 0 ($\mu_k$ = 0). 
A model that gives constraint $k$ a weight of 1.5 will accrue a lower cost than one that gives it a weight of 3.5, as the former has a smaller weight discrepancy.
More specifically, the value that is subtracted form the log likelihood is directly proportional to the square of the weight discrepancy, all else being equal (*i.e.* value to be subtracted $\propto (w_k - \mu_k)^2$).

(2) Cost increases as the tolerance for weight variation decreases. 
...
This same weight discrepancy incurs a higher cost in a model that has $\sigma_k$ = 0.5 (lower tolerance) than in a model that has $\sigma_k$ = 1 (higher tolerance).
More specifically, the value that is subtracted from the log likelihood is inversely proportional to the square of the tolerance, all else being equal (*i.e.* value to be subtracted $\propto (\frac{1}{\sigma_k})^2$).
...
# Take a look at structure of bias_file
...
The user, however, is advised that when multiple values for the same parameter is specified, the algorithm decides which one to use according to the chart in *Figure 3*.

```{r, echo=FALSE, fig.cap="Fig 3: Hierarchy for choosing which bias parameters to use when conflicts arise. Parent over-writes child.", out.width = '25%'}
knitr::include_graphics("../man/figures/bias_param_hierarchy.png")
```
...
Recall that `simple_model` attained a log likelihood of -1.386.
``` {r}
# Get log likelihood of simple model
simple_model$loglik
```

This log likelihood is in fact at ceiling. 
We know that because each data set has a theoretical maximum likelihood, which can be calculated.
...
* For Input1, there were 2 outputs, each of which surfaced once. 
  + Given Input1, the probability of each surfaced output is $.5$.
* For Input2, 1 output surfaced once. 
  + Given Input2, the probability of its lone surfaced output is $1$.

Thus the maximum attainable log likelihood is: $ln(.5) + ln(.5) + ln(1)= -1.386$, which happens to be what the trained model attained.
...
Since every constraint has the same ideal weight & the same standard deviation (tolerance for weight discrepancy), we can make use of the optional arguments `mu_scalar` & `sigma_scalar`.
These arguments allow us to input $\mu$ and $\sigma$ just once for all constraints.
```{r scalarBiases}
# Train model with mu_scalar=0 & sigma_scalar=1
muS_sigS_bias_model = optimize_weights(data_file,
                                       mu_scalar=0, sigma_scalar=1)
```

Inspecting the trained model, we now see a value for the attribute `bias_params`:
```{r}
# Take a took at the model trained with scalar biases
muS_sigS_bias_model
```

We can pull up this attribute alone:
```{r}
# Take a look at the bias parameters we used during model training
muS_sigS_bias_model$bias_params
```
...
Let's take a look at the trained weights for this simple bias model:
```{r biasModelAttributes}
# Get learned weights for the model trained with scalar biases
muS_sigS_bias_model$weights
```
We see that the trained model has learned that Constraint 1 has weight = 0.105, and Constraint 2 has weight = 0.316.
...
We attained this by including a prior term that increased the cost of weights as their distance from `mu_scalar` increased. 

We also see that the log likelihood of this model is lower than ceiling ($-2.000 < -1.386$), so overfitting is less likely to occur with this regularized model.

```{r biasModelLoglik}
# Get log likelihood of model trained with scalar biases
muS_sigS_bias_model$loglik
```

#### Pulling out the weight for 1 constraint
...
We use index `1` to pick out the first constraint in the named list `your_model_name$weights`.
...
cons1_simpleBias <- muS_sigS_bias_model$weights[1]

# Compare learned weights of Constraint 1 in the unbiased model and in the regularized one
...
Extension to untrained sounds occurred more frequently in the experimental condition (trained on p&rarr;v; extended to b&rarr;v) at 73% than in the control condition (trained on b&rarr;v; txtended to p&rarr;v) at 20%. The percentage alternation by participants in the response phase is shown below. 
...
We'll set $\mu$'s of these constraints to the following preferred weights:
...
white_salt_data_file <- system.file("extdata", "sample_whiteSalt_data_file.txt", package = "maxent.ot")
```

We now train the bias model for the experimental condition, making use of the argument `mu_vector`.
We want the value of $\mu$ to be 0, 0, 1.3 and 3.65 for the first four constraints respectively.
...
white_salt_bias_model = optimize_weights(white_salt_data_file,
                                       mu_vector=c(0, 0, 1.3, 3.65), sigma_scalar=sqrt(0.6))

# Look at trained weights (expt cdn with bias)
...
white_salt_noBias_model = optimize_weights(white_salt_data_file,
                                       mu_vector=c(0, 0, 2.27, 2.27), sigma_scalar=sqrt(0.6))

# Look at trained weights (expt cdn, no bias)
...
We'll take a closer look at both the function `predict_probabilities()`, and the structure of the test file in $\S4$ below.
...
white_salt_test_file <- system.file("extdata", "sample_whiteSalt_test_file.txt", package = "maxent.ot")

# Predict probabilities with weights trained with bias (expt cdn)
predict_probabilities(white_salt_test_file, white_salt_bias_model$weights)

# Predict probabilities with weights trained without bias (expt cdn)
predict_probabilities(white_salt_test_file, white_salt_noBias_model$weights)
```
...
```{r}
# Get path to data file (experimental condition)
white_control_data_file <- system.file("extdata", "sample_whiteCtrl_data_file.txt", package = "maxent.ot")

# Fit model with preferred weights for control condition
white_control_bias_model = optimize_weights(white_control_data_file,
                                       mu_vector=c(0, 0, 1.3, 3.65), sigma_scalar=sqrt(0.6))

# Look at trained weights (control cdn with bias)
white_control_bias_model$weights
```

```{r}
# Fit unbiased model for control condition
white_control_noBias_model = optimize_weights(white_control_data_file,
                                       mu_vector=c(0, 0, 2.27, 2.27), sigma_scalar=sqrt(0.6))

# Look at trained weights (control cdn, no bias)
white_control_noBias_model$weights
```

As with the experimental condition above, we'll now subject the two trained models to the response phase, which includes the test (b&rarr;v) and (p&rarr;v) extension tasks.

```{r}
# Get path to test file (control condition)
white_control_test_file <- system.file("extdata", "sample_whiteCtrl_test_file.txt", package = "maxent.ot")

# Predict probabilities with weights trained with bias (control cdn)
predict_probabilities(white_control_test_file, white_control_bias_model$weights)

# Predict probabilities with weights trained with bias (control cdn)
predict_probabilities(white_control_test_file, white_control_noBias_model$weights)
```
...
The biased model performs relatively well.
The unbiased model, in contrast, greatly overgeneralizes to the untrained p&rarr;v alternation.

### 3.4.4 Conclusion 
Overall, the biased model performed much better than the unbiased one at modeling human subject responses during the response phase.
The unbiased model both underpredicted extension to the b&rarr;v alternation in the experimental condition and overpredicted the extension to the p&rarr;v alternation in the control condition.
...
The resulting trained biased model was able to match the observed frequencies of alternations produced by human subjects during the response phase much more closely than its unbiased counterpart in all experimental conditions. 
...
If we've trained the model using `optimize_weights()` and stored it in the object `fit_model`, we can specify the constraint weights by passing in the `weights` attribute of the model: `fit_model$weights`. 
...
The first subject picked *Output1-1* for *Input1* and *Output2-1* for *Input2*.
The second subject picked *Output1-2* for *Input1* and failed to respond for *Input2*.
...
For this reason, it is strongly recommended to check one's data set for duplicate URs, in order to collapse duplicate URs to a single UR if the user desires to make use of the `Error` values.
...
# Be sure to order them in exactly the same order in which they appear in the input file!
...
As before, $f_k(SR_i, UR_i)$ is the number of violations of constraint $k$ incurred by mapping underlying $UR_i$ to surface $SR_i$, $Z_w(UR_i)$ is a normalization term defined as 

$Z(UR_i) = \Sigma_{y\in \mathcal{Y}(UR_i)} exp(- \Sigma_{k=1}^m \frac{w_k f_k(SR, UR_i)}{T})$
...
Thus, all the cases that we've encountered in the preceding sections when we applied `predict_probabilities()` or `optimize_weights()` can be assumed to have $T=1$, since we haven't used the argument `temperature` yet.
Notice that when $T=1$, the generalized model is equivalent to the simplified one defined in $\S1.2$.
...
For example, if a particular UR has two SRs, higher values of $T$ will move the probability of each one towards $0.5$.
...
First, notice that we can extract the predicted probabilities like so:
``` {r}
...
* at $T=1$, 73% *Output1-1*, 27% *Output1-2*
* at $T=3$, 58% *Output1-1*, 42% *Output1-2*
* at $T=5$, 55% *Output1-1*, 45% *Output1-2*

Thus, as temperature increases, we see that the predicted probabilities of the outputs of a given input become less polarized.
That is, they move closer towards equiprobability with each other.
...
hu_data <- system.file("extdata", "sample_hu_wug.txt", package = "maxent.ot")
...
names(lex_wts_ls) <- list('AGREE(back,nonlocal)',	'AGREE(front,local)',	
...
hu_pred <- predict_probabilities(hu_data, lex_wts, temperature = 1.5, encoding = "UTF-8")
...
hu_pred_t1 <- predict_probabilities(hu_data, lex_wts, encoding = "UTF-8")
...
hu_500simul_wts_path <- system.file("extdata", "hu_500simuls_wts.txt", package = "maxent.ot")
...
# Rename columns
names(hu_500simul_wts) <- c('AGREE(back,nonlocal)',	'AGREE(front,local)',
                            'AGREE(nonhigh_front,local)',	'AGREE(low_front,local)',
                            'AGREE(double_front,local)',	'USE_FRONT/bilabial__',
                            'USE_FRONT/[+cor,+son]__', 'USE_FRONT/sibilant__',
                            'USE_FRONT/CC__',	'USE_BACK/[C0i:C0]__')
...
The caution against duplicate UR entries highlighted in $\S4.1$ doesn't apply here because the `Error` column of the `hu_pred` file isn't used by `monte_carlo_weights()`. 
...
small_data <- system.file("extdata", "sample_data_file_small.txt", package = "maxent.ot")
# This file has three constraints
large_data <- system.file("extdata", "sample_data_file_large.txt", package = "maxent.ot")
...
# Get paths to toy data files
small_data_b_old <- system.file("extdata", "sample_data_file_small_b_old.txt", package = "maxent.ot")
large_data_b_old <- system.file("extdata", "sample_data_file_large_b_old.txt", package = "maxent.ot")
...
``` {r include=FALSE}
# TO REMOVE: here just for troubleshooting
large_model$loglik
small_model$loglik

2*(large_model$loglik - small_model$loglik)
```
...
The `aic.wt` for a current model, $w_{aic}$, is just its raw AIC weight normalized over the sum of raw AIC weights of all other models that were passed in. 
...
where $\delta$ is the difference between the current model's AIC score and the best model's AIC score.
$\delta = 0$ for the best model, and $\delta \gt 0$ for the rest of the models.
Hence, the best model's $r$ is $e^0 = 1$, and the worse models will have $r \lt 1$.


## 6.3 Bayesian Information Criterion (BIC)
### 6.3.1 BIC: Background

The BIC is an alternative measure of assessing the trade-off between the number of parameters and model fit.
...
small_data_c <- system.file("extdata", "sample_data_file_small_c.txt", package = "maxent.ot")

# This file has 3 constraints
large_data_c <- system.file("extdata", "sample_data_file_large_c.txt", package = "maxent.ot")
...
# Get paths to toy data files
# This file has 2 constraints
small_data_b <- system.file("extdata", "sample_data_file_small_b.txt", package = "maxent.ot")
# This file has 3 constraints
med_data_b <- system.file("extdata", "sample_data_file_med_b.txt", package = "maxent.ot")
# This file has 4 constraints
large_data_b <- system.file("extdata", "sample_data_file_large_b.txt", package = "maxent.ot")
